
## simulate data for testing TrialEmulation package, using the algorithm in Young and Tchetgen Tchetgen (2014) 


DATA_GEN_continous_timevarying_outcome_treatment_switch<-function(ns, nv, conf = 0.5, treat_prev = 0,
                                                      outcome_prev = -3.8, all_treat = FALSE,
                                                      all_control = FALSE, censor = TRUE){   
  # ns= number of subjects, nv=no of visits including baseline visit
  
  
  nvisit<-nv+1
  
  X1<-rep(0,nvisit*ns)          ## place holders for time-varying covariates
  Z1<-rnorm(nvisit*ns,0,0.5)
  X2<-rep(0,nvisit*ns)    
  Z2<-rnorm(nvisit*ns,0,0.5)
  
  CX1 <- rep(0,nvisit*ns)
  CX2 <- rep(0,nvisit*ns)
  CX1p <- rep(0,nvisit*ns)
  CX2p <- rep(0,nvisit*ns)
  
  A<-rep(0,nvisit*ns) ##place holders for current  treatments
  Ap<-rep(0,nvisit*ns) ##place holders for  previous treatments
  CA<- rep(0,nvisit*ns)
  CAp<-rep(0,nvisit*ns)   ##place holders for sum of previous treatment A
  
  
  
  Y<-rep(0,nvisit*ns)     ##place holders for outcome vector
  Yp<-rep(0,nvisit*ns)    ##place holders for previous outcome vector
  
  ##Fill in initial values
  seq1<-seq(1,nvisit*ns-nv,nvisit)
  
  X1[seq1]<-0
  X2[seq1]<-0
  CX1[seq1]<-0
  CX2[seq1]<-0
  CX1p[seq1]<-0
  CX2p[seq1]<-0
  P1<-list() ##list of treatment probabilities 
  P1[[1]]<-rep(0, ns) 
  seqlist<-list()                              
  seqlist[[1]]<-seq1
  CA[seq1]<-rep(0, ns)
  CAp[seq1]<-rep(0, ns)
  
  for (k in 2:nvisit){  
    ## update covariates
    
    
    seqlist[[k]]<-seqlist[[k-1]]+1
    Ap[seqlist[[k]]]<-A[seqlist[[k-1]]]
    CX1p[seqlist[[k]]]<-CX1p[seqlist[[k-1]]] + X1[seqlist[[k-1]]]
    CX1p[seqlist[[k]]]<-CX2p[seqlist[[k-1]]] + X2[seqlist[[k-1]]]
    
    CAp[seqlist[[k]]]<-CAp[seqlist[[k-1]]]+Ap[seqlist[[k]]]
    
    X1[seqlist[[k]]]<-Z1[seqlist[[k]]]-0.3*Ap[seqlist[[k]]] ## continuous time-varying confounder 
    X2[seqlist[[k]]]<-Z2[seqlist[[k]]]-0.3*Ap[seqlist[[k]]]
    
    CX1[seqlist[[k]]]<- CX1[seqlist[[k-1]]] + X1[seqlist[[k]]]
    CX2[seqlist[[k]]]<-CX2[seqlist[[k-1]]] + X2[seqlist[[k]]]
    ## update treatment
    
    ########### Old formula: lpp<- as.numeric(treat_prev) + Ap[seqlist[[k]]]+0.5*X1[seqlist[[k]]]+as.numeric(conf)*X1[seqlist[[k]]]
    ###########                    -0.2*X3[seqlist[[k]]]+X2[seqlist[[k]]]-0.3*(age[seqlist[[k]]]-35)/12
    
    lpp<- 2.5*Ap[seqlist[[k]]] -2.5*(1-Ap[seqlist[[k]]]) + as.numeric(conf)*(X1[seqlist[[k]]]+X2[seqlist[[k]]]) 
    P1[[k]]<-1/(1+exp(-lpp))
    
    if (all_treat == TRUE){
      A[seqlist[[k]]]<- 1.0
    } else{ if (all_control == TRUE){
      A[seqlist[[k]]]<- 0.0
    } else{ if (k == 2){
      A[seqlist[[k]]]<-rbinom(ns,1,as.numeric(treat_prev))
    } else{
      A[seqlist[[k]]]<-rbinom(ns,1,P1[[k]])}
    }
    }
    ##Generate outcome
    
    Yp[seqlist[[k]]]<-Y[seqlist[[k-1]]]
   
    Y[seqlist[[k]]]<- 50 + 5*(CX1[seqlist[[k]]] + CX2[seqlist[[k]]]) -3*A[seqlist[[k]]]+ rnorm(ns,0,10)
    
    
    
  }
  
  ##Make data frame
  
  ID<-rep(1:ns,each=nv)
  
  ##Align data by removing values 
  NSEQ<-seq1
  
  X1<-X1[-NSEQ]
  X2<-X2[-NSEQ]
  CX1<-CX1[-NSEQ]
  CX2<-CX2[-NSEQ]
  A<-A[-NSEQ]
  Ap<-Ap[-NSEQ]
  CAp<-CAp[-NSEQ]
  CA<-CA[-NSEQ]
  Y<-Y[-seq(1,nvisit*ns-nv,nvisit)]
  Yp<-Yp[-seq(1,nvisit*ns-nv,nvisit)]
  
  ##Create data frame
  
  DATA<-data.frame(ID,t=rep(c(0:(nv-1)),ns),A,Ap,CAp,X1,X2,CX1, CX2, Y)
  DATA$eligible<-as.numeric(CAp==0)  ## eligibility criteria: age>=18, had no treatment so far, no event so far
  
  ##censoring
  if (censor == T){
    Dprob<-1/(1+exp(2.5 + Ap-0.5*X1-0.2*X2)) ##Probability of dropout
    
    DATA$C<-rbinom(nv*ns,1,Dprob) ##C=0 is remain in the study
    
    
    
    indfun<-function(n){
      if (sum(n)==0) {rep(0,nv)}
      else{k<-min(which(n==1))
      c(rep(0,k),rep(1,nv-k))}}
    
    RL<-ave(DATA$C,DATA$ID,FUN=indfun)
    
    
    eligCum<-ave(DATA$eligible,DATA$ID,FUN=cumsum)
    
    DATA[RL==0 & eligCum>0,] #remove observations after event occurrence and censoring, and not eligible
  } else {
    DATA$C <- 0
    eligCum<-ave(DATA$eligible,DATA$ID,FUN=cumsum)
    
    DATA[eligCum>0,] 
  }
  
}
