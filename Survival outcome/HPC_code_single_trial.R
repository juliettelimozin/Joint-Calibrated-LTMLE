...
objectives <- array(,dim = c(2,8,iters))
hr_estimates <- array(,dim = c(2,iters))
mr_estimates <- array(,dim = c(2,2,5,iters))
meandiffs <- array(, dim = c(3,4,6,iters))
...
for (i in 1:iters){
  tryCatch({
    ...
    PP <- TrialEmulation::trial_msm(data = switch_data,
                                    outcome_cov = ~ X1 + X2+ X2sq + assigned_treatment+
                                      t_1 + t_2 + t_3 + t_4 +
                                      t_1A + t_2A + t_3A + t_4A +
    ...
    fitting_data_treatment <-  switch_data %>%
      ...
      dplyr::select(id,trial_period, followup_time, X1,  X2, X2sq, assigned_treatment) %>%
      merge(design_mat, by = c("id", "trial_period", "followup_time"), all.y = TRUE) %>%
      dplyr::group_by(id) %>%
      tidyr::fill( X1,X2,X2sq, assigned_treatment,.direction = "down") %>%
      dplyr::ungroup() %>%
      dplyr::select(id, trial_period, followup_time, X1, X2, X2sq, assigned_treatment) %>%
...
save(meandiffs, file = paste("Simulation results/meandiffs_singletrial_low_scenario1_",as.character(l),".rda", sep = ""))
save(objectives, file = paste("Simulation results/objectives_singletrial_low_scenario1_",as.character(l),".rda", sep = ""))
save(hr_estimates, file = paste("Simulation results/hr_estimates_singletrial_low_scenario1_",as.character(l),".rda", sep = ""))
save(mr_estimates, file = paste("Simulation results/mr_estimates_singletrial_low_scenario1_",as.character(l),".rda", sep = ""))
