covariate_summary <- function(data, covariates, factor_vars = NULL, digits = 1) {
    ...
    if (is.numeric(x)) {
      data.frame(
        ...
        summary = paste0(
          ...
          " (",
          round(quantile(x, 0.25, na.rm = TRUE), digits),
          ", ",
...
covariate_table_A <- function(data, covariates, A,
                              factor_vars = NULL,
                              digits = 1) {
  df <- data[, c(A, covariates), drop = FALSE]
  df[[A]] <- as.factor(df[[A]])
  ...
  tables <- lapply(covariates, function(v) {
    x <- df[[v]]

    # ---------- Continuous ----------
    if (is.numeric(x)) {
      stats <- sapply(levels(df[[A]]), function(a) {
        z <- x[df[[A]] == a]
        paste0(
          round(median(z, na.rm = TRUE), digits),
          " (",
          round(quantile(z, 0.25, na.rm = TRUE), digits),
          ", ",
          round(quantile(z, 0.75, na.rm = TRUE), digits),
          ")"
        )
      })
      ...
      data.frame(
        ...
        level = rownames(tab),
        `A = 1` = paste0(
          tab[, "1"], " (", round(pct[, "1"], digits), "%)"
        ),
        `A = 0` = paste0(
          tab[, "0"], " (", round(pct[, "0"], digits), "%)"
        ),
        row.names = NULL
      )
    }
  ...
  for (m in 0:4) {
    if (m != 0) {
      data[data$t == m & data$A == 1, ]$hs <- as.vector(predict.glm(treat_models_n[[m + 1]],
        ...
        type = "response"
      ) * (1 - predict.glm(censor_models_n[[m]],
        newdata = data[data$t == m & data$A == 1, ],
        type = "response"
      )))
...
initial_weight_calculation <- function(simdata) {
  ...
  for (m in 0:4) {
    simdata[simdata$t == m & simdata$A == 1, ]$ps <- as.vector(predict.glm(treat_models_d[[m + 1]],
      ...
      type = "response"
    ) / predict.glm(treat_models_n[[m + 1]],
      newdata = simdata[simdata$t == m & simdata$A == 1, ],
      type = "response"
    ))
    simdata[simdata$t == m & simdata$A == 0, ]$ps <- as.vector((1 - predict.glm(treat_models_d[[m + 1]],
      ...
      type = "response"
    )) / (1 - predict.glm(treat_models_n[[m + 1]],
      newdata = simdata[simdata$t == m & simdata$A == 0, ],
      type = "response"
    )))
    if (m != 0) {
      simdata[simdata$t == m, ]$pr_c <- as.vector((1 - predict.glm(censor_models_d[[m]],
        newdata = data.frame(
          sqrtCD4_1 = simdata[simdata$t == m, ]$lagsqrtCD4_1,
...
fit_msm <- function(wideSimdata,
  ...
  rownames(EY_always_treated_strata0) <- rownames(EY_always_treated_strata1) <- rownames(EY_always_treated_strata2) <- c(
    "MLE LTMLE",
    "CMLE LTMLE treat. only",
    "Aggr. CMLE LTMLE treat. only",
    "CMLE LTMLE",
  ...
  rownames(EY_never_treated_strata0) <- rownames(EY_never_treated_strata1) <- rownames(EY_never_treated_strata2) <- c(
    "MLE LTMLE",
    "CMLE LTMLE treat. only",
    "Aggr. CMLE LTMLE treat. only",
    "CMLE LTMLE",
  ...
  counterfactual_means <- list(
    EY_always_treated_strata0 = EY_always_treated_strata0,
...
MyLtmleMSM_modified_hers <- function(simdata, initial_Q_t = NA, refit_weights = TRUE, treat_models_n = NA, censor_models_n = NA) {
  ...
  a <- min(simdata$CD4, na.rm = TRUE)
  b <- max(simdata$CD4, na.rm = TRUE)
  ...
  for (k in 0:4) {
    for (j in k:0) {
      if (k == j) {
        ...
        logitQforms[[(k + 1), (j + 1)]] <- as.matrix(c(
          qlogis((predict.glm(Qform_d1,
            newdata = Q_predict_data_d1
          ) - a) / (b - a)),
          qlogis((predict.glm(Qform_d0,
            newdata = Q_predict_data_d0
          ) - a) / (b - a))
        ))
      }
    }
  ...
  for (k in 0:4) {
    for (j in k:0) {
      ...
      if (k == j) {
        ...
        Qstar_fit <- glm(
          formula = as.formula(formula_string), family = "quasibinomial",
        ...
        Qstar_fit_cali_aggr_treat_only <- glm(
          formula = as.formula(formula_string), family = "quasibinomial",
          data = update_data[regimen_ind == 1, ],
        ...
        Qk_j_star_fit <- glm(
          Qk_j1star ~ intercept_0 + intercept_1 +
            intercept_2 + intercept_3 + intercept_4 +
        ...
        Qk_j_star_fit_cali <- glm(
          Qk_j1star_cali ~ intercept_0 + intercept_1 +
            intercept_2 + intercept_3 + intercept_4 +
            D1 + D2 + D0:cumA + D1:cumA + D2:cumA +
            offset(off) - 1,
          family = "quasibinomial",
        ...
        Qk_j_star_fit_cali_aggr <- glm(
          Qk_j1star_cali_aggr ~ intercept_0 + intercept_1 +
            intercept_2 + intercept_3 + intercept_4 +
            D1 + D2 + D0:cumA + D1:cumA + D2:cumA +
            offset(off) - 1,
          family = "quasibinomial",
        ...
        Qk_j_star_fit_cali_treat_only <- glm(
          Qk_j1star_cali_treat_only ~ intercept_0 + intercept_1 +
            intercept_2 + intercept_3 + intercept_4 +
            D1 + D2 + D0:cumA + D1:cumA + D2:cumA +
            offset(off) - 1,
          family = "quasibinomial",
        ...
        Qk_j_star_fit_cali_aggr_treat_only <- glm(
          Qk_j1star_cali_aggr_treat_only ~ intercept_0 + intercept_1 +
            intercept_2 + intercept_3 + intercept_4 +
            D1 + D2 + D0:cumA + D1:cumA + D2:cumA +
            offset(off) - 1,
          family = "quasibinomial",
  ...
  list(
    MSM_estimates = msm_fit$MSM_estimates,
...
MyLtmleMSM_hers <- function(simdata) {
  ...
  for (k in 0:4) {
    for (j in k:0) {
      ...
      if (k == j) {
        formula_string <- paste0(
          "Y_", j, "_scaled ~ A_", j, "+ Ap_", j, " + App_", j,
          "+ sqrtCD4_1_", j, " + sqrtCD4_2_", j,
          " + viral_", j, " + viral_1_", j, " + viral_2_", j,
          " + HIVsym_", j, " + HIVsym_1_", j, " + HIVsym_2_", j,
          " + SITE2_", j, " + SITE3_", j, " + WHITE_", j, " + OTHER_", j
        ...
        Qfit <- glm(data = wideSimdata, formula = as.formula(formula_string), family = 'quasibinomial')

        logitQforms[[(k + 1), (j + 1)]] <- predict.glm(Qfit, newdata = predict_data, type = 'link')
        ...
        Qform_d1 <- glm(data = fitting_data[1:dim(wideSimdata)[1], ], 
                        ...
                        family = 'quasibinomial')
        Qform_d0 <- glm(data = fitting_data[(dim(wideSimdata)[1] + 1):(dim(wideSimdata)[1] * 2), ], 
                        formula = as.formula(formula_string),
                        family = 'quasibinomial')
        ...
        Qform_d1_cali <- glm(data = fitting_data[1:dim(wideSimdata)[1], ], 
                             formula = as.formula(formula_string_cali),
                             family = 'quasibinomial')
        Qform_d0_cali <- glm(data = fitting_data[(dim(wideSimdata)[1] + 1):(dim(wideSimdata)[1] * 2), ], 
                             formula = as.formula(formula_string_cali),
                             family = 'quasibinomial')

        logitQforms_cali[[(k + 1), (j + 1)]] <- as.matrix(c(
          predict.glm(Qform_d1_cali,
            newdata = Q_predict_data_d1,
            type = 'link'),
        ...
        Qform_d1_cali_aggr <- glm(data = fitting_data[1:dim(wideSimdata)[1], ], 
                                  formula = as.formula(formula_string_cali_aggr),
                                  family = 'quasibinomial')
        Qform_d0_cali_aggr <- glm(data = fitting_data[(dim(wideSimdata)[1] + 1):(dim(wideSimdata)[1] * 2), ], 
                                  formula = as.formula(formula_string_cali_aggr),
                                  family = 'quasibinomial')

        logitQforms_cali_aggr[[(k + 1), (j + 1)]] <- as.matrix(c(
          predict.glm(Qform_d1_cali_aggr,
            newdata = Q_predict_data_d1,
            type = 'link'),
        ...
        Qform_d1_cali_treat_only <- glm(data = fitting_data[1:dim(wideSimdata)[1], ], 
                                        formula = as.formula(formula_string_cali_treat_only),
                                        family = 'quasibinomial')
        Qform_d0_cali_treat_only <- glm(data = fitting_data[(dim(wideSimdata)[1] + 1):(dim(wideSimdata)[1] * 2), ], 
                                        formula = as.formula(formula_string_cali_treat_only),
                                        family = 'quasibinomial')

        logitQforms_cali_treat_only[[(k + 1), (j + 1)]] <- as.matrix(c(
          predict.glm(Qform_d1_cali_treat_only,
            newdata = Q_predict_data_d1,
            type = 'link'),
          predict.glm(Qform_d0_cali_treat_only,
            newdata = Q_predict_data_d0,
            type = 'link')
        ))

        Qform_d1_cali_aggr_treat_only <- glm(data = fitting_data[1:dim(wideSimdata)[1], ], 
                                             formula = as.formula(formula_string_cali_aggr_treat_only),
                                             family = 'quasibinomial')
        Qform_d0_cali_aggr_treat_only <- glm(data = fitting_data[(dim(wideSimdata)[1] + 1):(dim(wideSimdata)[1] * 2), ], 
                                             formula = as.formula(formula_string_cali_aggr_treat_only),
                                             family = 'quasibinomial')

        logitQforms_cali_aggr_treat_only[[(k + 1), (j + 1)]] <- as.matrix(c(
          predict.glm(Qform_d1_cali_aggr_treat_only,
            newdata = Q_predict_data_d1,
            type = 'link'),
          predict.glm(Qform_d0_cali_aggr_treat_only,
            newdata = Q_predict_data_d0,
            type = 'link')
        ))
