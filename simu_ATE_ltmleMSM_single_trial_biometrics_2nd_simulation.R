...
simulation <- foreach(i = 1:iters, .combine=cbind) %dopar% {
  ...
  suppressMessages(suppressWarnings({
    simdata<-DATA_GEN(ns = sample_size, nv = nv, conf = conf)
...
cat(paste('Everything is correctly specified'))
...
results <- cbind(rowMeans(simulation, na.rm = TRUE)[25:28]-200, rowMeans(simulation, na.rm = TRUE)[29:32]-10,
                 rowSds(simulation, na.rm = TRUE)[25:28], rowSds(simulation, na.rm = TRUE)[29:32],
                 sqrt((rowMeans(simulation, na.rm = TRUE)[25:28]-200)^2+ rowSds(simulation, na.rm = TRUE)[25:28]^2),
                 sqrt((rowMeans(simulation, na.rm = TRUE)[29:32]-10)^2+ rowSds(simulation, na.rm = TRUE)[29:32]^2))

print(xtable(results, type = "latex"))

cat(paste('Estimation of beta_0 (true value = 200)\n'))
cat('Bias: \n')
print(rowMeans(simulation, na.rm = TRUE)[25:28]-200, digits = 3)

cat('SD: \n')
print(rowSds(simulation, na.rm = TRUE)[25:28], digits = 3)

cat('rootMSE: \n')
print(sqrt((rowMeans(simulation, na.rm = TRUE)[25:28]-200)^2+ rowSds(simulation, na.rm = TRUE)[25:28]^2), digits = 3)

cat(paste('Estimation of beta_1 (true value = 10)\n'))
cat('Bias: \n')
print(rowMeans(simulation, na.rm = TRUE)[29:32]-10, digits = 3)

cat('SD: \n')
print(rowSds(simulation, na.rm = TRUE)[29:32], digits = 3)

cat('rootMSE: \n')
print(sqrt((rowMeans(simulation, na.rm = TRUE)[29:32]-10)^2+ rowSds(simulation, na.rm = TRUE)[29:32]^2), digits = 3)
