...
simdata_censored<-DATA_GEN_censored_reduced(1000,5, conf = 2.5, censor = F) 
...
bal.tab(simdatafinal$data[simdatafinal$data$t == 1,c('X2', 'X4')],treat = simdatafinal$data[simdatafinal$data$t == 1,]$A,
        stats = c('m', 'v'),var.name = c('X2', 'X4'))
bal.plot(simdatafinal$data[simdatafinal$data$t == 1,],stats = c('m', 'v'),var.name = 'X2',
         treat = simdatafinal$data[simdatafinal$data$t == 1,]$A )

bal.tab(simdatafinal$data[simdatafinal$data$t == 1,c('X2', 'X4')],treat = simdatafinal$data[simdatafinal$data$t == 1,]$A,
        stats = c('m', 'v'),var.name = c('X2', 'X4'), weights = simdatafinal$data[simdatafinal$data$t == 1,]$weights )
bal.plot(simdatafinal$data[simdatafinal$data$t == 1,],stats = c('m', 'v'),var.name = 'X2',
         treat = simdatafinal$data[simdatafinal$data$t == 1,]$A, weights = simdatafinal$data[simdatafinal$data$t == 1,]$weights )

bal.tab(simdatafinal$data[simdatafinal$data$t == 1,c('X2', 'X4')],treat = simdatafinal$data[simdatafinal$data$t == 1,]$A,
        stats = c('m', 'v'),var.name = c('X2', 'X4'), weights = simdatafinal$data[simdatafinal$data$t == 1,]$Cweights )
bal.plot(simdatafinal$data[simdatafinal$data$t == 1,],stats = c('m', 'v'),var.name = 'X2',
         treat = simdatafinal$data[simdatafinal$data$t == 1,]$A, weights = simdatafinal$data[simdatafinal$data$t == 1,]$Cweights )
...
ggplot() +geom_line(aes(x = 0:4,y = pull(predicted_probas_PP,mrd), 
                        color = 'MLE IPW'))+
  geom_point(aes(x = 0:4,y = pull(predicted_probas_PP,mrd)))+ 
  geom_line(aes(x = 0:4,y = pull(predicted_probas_PP,mrd_cali), 
                 color = 'Calibrated IPW'))+
  geom_point(aes(x = 0:4,y = pull(predicted_probas_PP,mrd_cali)))+ 
  scale_color_manual(name = "Weighting method",
                     values = c("MLE IPW"= "red",
                                "Calibrated IPW" = "blue")) +
  labs(x = 'Follow-up time', 
       y = "Marginal risk difference", title = "Comparing MRD estimates in a single trial case")
