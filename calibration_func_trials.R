...
NRroot <- function(inipar, infcn, nmax = 25, stoptol = 1e-05,
  ...
  result <- list(
    nstep = ctr, initial = inipar, final = newpar,
...
calibration <- function(simdatafinal, var = c("tall", "X1", "X2", "X3", "X4")) {
  ...
  weioptAR <- nleqslv::nleqslv(rep(0, dim(DMATR2)[2] - 1), gfunAR, DgAR,
    method = "Broyden",
...
calibration_by_time_from_baseline <- function(simdatafinal,
  ...
  for (k in 1:T) {
    ...
    if (isTRUE(k >= subset_t)) {
      data1RA <- cbind(data1RA$RA, data1RA$RA * data1RA[, subset_var])
    } else {
    ...
    if (censor == TRUE) {
      if (isTRUE(k >= subset_t)) {
        data1RC <- cbind(simdatafinal[simdatafinal$tall == k, ]$lagRA, simdatafinal[simdatafinal$tall == k, ]$lagRA * simdatafinal[simdatafinal$tall == k, subset_c_var])
      } else {
    ...
    if (isTRUE(k >= subset_t)) {
      data0RA <- cbind((1 - data0RA$C) * data0RA$RA * data0RA$Cweights, (1 - data0RA$C) * data0RA$RA * data0RA$Cweights * data0RA[, subset_var_lead])
    } else {
    ...
    if (censor == TRUE) {
      ...
      if (isTRUE(k >= subset_t)) {
        data0RC <- cbind(data0RC$RA * data0RC$Cweights, data0RC$RA * data0RC$Cweights * data0RC[, subset_c_var_lead]) # RA_k-1*RC_k-1*W_k-1*H_k-1
      } else {
...
aggregated_calibration_from_baseline <- function(simdatafinal, calibrate_treat = TRUE, var = c("X1", "X2", "X3", "X4"), censor = FALSE, c_var = NULL, weights_var = "weight") {
  ...
  if (censor == TRUE) {
    ...
    if (calibrate_treat) {
      Baselinecond <- c(
        maxlen * colSums(DMATR2_RA[indT0, ]), # (T+1)sum_{i = 1}^N X_0
        rep(0, dim(DMATR2_RC[indT0, ])[2])
      ) # 0
    } else {
  ...
  weioptAR <- nleqslv::nleqslv(rep(0, dim(DMATRO)[2]), gfunAR, DgAR,
    method = "Broyden",
    control = list(maxit = 10000, ftol = 10^(-16), xtol = 10^(-16)), jacobian = T
  )
